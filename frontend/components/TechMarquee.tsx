...
/**
 * Infinite-scroll tech logo marquee.
 * Self-contained CSS animation — no external stylesheet dependency.
 * Icons from Simple Icons CDN (~1-2 KB SVG each).
 */
  ...
  return (
    <div
      ...
      onMouseLeave={(e) => {
        ...
        const img = e.currentTarget.querySelector("img");
        const span = e.currentTarget.querySelector("span");
        if (img) { img.style.opacity = "0.5"; img.style.filter = "brightness(0.8)"; }
        if (span) span.style.color = "var(--color-text-muted)";
      }}
    >
      <img
        src={`https://cdn.simpleicons.org/${slug}/999999`}
        alt={name}
        width={20}
        height={20}
        loading="lazy"
        style={{ opacity: 0.5, filter: "brightness(0.8)", transition: "all 0.3s ease" }}
...
/* Inline AWS SVG since the CDN slug is unreliable */
function AwsItem() {
  return (
    <div
      style={{
